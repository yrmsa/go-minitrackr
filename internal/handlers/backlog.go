...
func (h *Handler) CreateBacklogIssue(w http.ResponseWriter, r *http.Request) {
	...
	h.templates.ExecuteTemplate(w, "issue-row", issue)
}

func (h *Handler) UpdateBacklogIssue(w http.ResponseWriter, r *http.Request) {
	id, err := parseID(r.URL.Path, "/backlog/issues/")
	if err != nil {
	...
	w.Header().Set("Content-Type", "text/html")
	h.templates.ExecuteTemplate(w, "issue-row", updated)
}

func (h *Handler) DeleteBacklogIssue(w http.ResponseWriter, r *http.Request) {
	id, err := parseID(r.URL.Path, "/backlog/issues/")
	if err != nil {
		http.Error(w, "Invalid ID", http.StatusBadRequest)
		return
